...
void test_mpi_2(int argc, char* argv[]) {
    ...
    auto rank = dolfinx::MPI::rank(MPI_COMM_WORLD);
    spdlog::warn("Rank: {}", rank);

    std::vector<int> in_values(rank + 1, rank);
    std::span<int>   in_values_span(in_values);
    std::vector<int> out_values;
    // npuheart::mpi::all_gather(MPI_COMM_WORLD, in_values, out_values);
    npuheart::mpi::gather(MPI_COMM_WORLD, in_values_span, out_values, 0);

    if (rank == 0) {
        for (size_t i = 0; i < out_values.size(); i++) {
            spdlog::warn("out_values[{}]: {}", i, out_values[i]);
        }
    }
}
...
int main_poisson(int argc, char* argv[])
{
  ...
  
  {
    ...
    auto range_b = _b.local_range();
    auto size_local = b.index_map()->size_local();
    auto size_ghost = b.index_map()->num_ghosts();
    spdlog::warn("size_local:  {}, size_ghost: {}.", size_local, size_ghost);
    spdlog::warn("range_b: {}, {}", range_b[0], range_b[1]);
